-- Based on https://github.com/project-ethea/Naia.git
-- Lifted from: Naia
...
---
local function wgettext(str, domain)
        if domain == nil then
                domain = "wesnoth"
        end
...
---
local function table_empty(table_v, include_nil)
...
---
local function table_keys(table_v)
        local keys = {}
...
local _ = wesnoth.textdomain "wesnoth-Wild_Frontiers"
...
function wf_do_amla_menu(cfg)
	...
	wesnoth.show_dialog(amla_dlg, function()
		...
		wesnoth.set_dialog_value(false, "preview_toggle")
...
--   [/wf_amla_list]
--
function wesnoth.wml_actions.wf_amla_list(cfg)
	-- [wf_amla_list] is not meant to modify the gamestate, but it makes heavy use
	...
	wesnoth.unsynced(function() wf_do_amla_menu(cfg) end)
end
